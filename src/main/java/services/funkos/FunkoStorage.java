...
public class FunkoStorage {
    private static FunkoStorage instance;
    private final IdGenerator idGenerator;
    private final Routes routes;
    private final Logger logger = LoggerFactory.getLogger(FunkoRepositoryImpl.class);


    private FunkoStorage() {
        idGenerator = IdGenerator.getInstance();
        routes = Routes.getInstance();
    }

    public static synchronized FunkoStorage getInstance() {
        if (instance == null) {
            instance = new FunkoStorage();
        }
        return instance;
    }
    ...
    public Mono<Void> exportJson(String ruta) {
        ...
        loadCsv().collectList().flatMap(funkos1 -> {
            funkos.addAll(funkos1);
            return Mono.empty();
        }).subscribe();
